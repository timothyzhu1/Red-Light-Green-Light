...
## Approach
Our agent’s performance and learning is based on the rewards and penalties we impose, considering the states of the lights and the forward and backward movement of the agent. More specifically, we reward the agent with +1 for every step forward that it takes, and we penalize the agent with -1 for every step backward that it takes. Additionally, the penalty for moving while the rightmost light is on is -200. This action will also end the mission and start the next episode. Similarly, the reward for the agent reaching the finish line is -100. To train our agent, we utilized the Proximity Policy Optimization (PPO) and Importance Weighted Actor Learning Architecture (IMPALA) algorithms from the RLlib, which are both effective in their own ways.Generally, both PPO and IMPALA use stochastic gradient descent, a quick and efficient way to iteratively train models. Additionally, we trained our agent on a system with multiple GPUs, and PPO enabled us to maximize the performance of this system, as we were able to distribute multiple workers to the different GPUs.
