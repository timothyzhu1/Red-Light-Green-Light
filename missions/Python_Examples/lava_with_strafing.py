...
class RedLightGreenLightGame(gym.Env):
    ...
    def __init__(self, env_config):
        ...
        self.observation_space = Box(-2, 3, shape=(5 * self.obs_size * self.obs_size, ), dtype=np.float32)
        self.currVelo = 0
        self.currStrafe = 0
    ...
    def step(self, action):
        def momentum(acceleration, strafe):
            #increase time val
            if not strafe:
                for i in range(10000):
                    ...
                    if self.currVelo < -1:
                        ...
                        self.currVelo = 1
                    command = f"move {self.currVelo}"
                    self.Game_Player.sendCommand(command)
                    time.sleep(0.0001)
            else:
                for i in range(10000):
                    self.currStrafe += acceleration * 0.0001
                    if self.currStrafe < 0:
                        self.currStrafe = 0
                    elif self.currStrafe > 1:
                        self.currStrafe = 1
                    command = f"strafe {self.currStrafe}"
                    self.Game_Player.sendCommand(command)
                    time.sleep(0.0001)

        # Get Action
        momentum(action[0], False)
        momentum(action[1], True)
        self.episode_step += 1
        ...
        if (len(np.argwhere(self.obs == 2)) >= 1):
            print("last light on")
            print("velo =", self.currVelo)
            print("strafe =", self.currStrafe)
            if self.currVelo != 0 or self.currStrafe != 0:
                print("lost")                
                done = True
        ...
        return '''<?xml version="1.0" encoding="UTF-8" standalone="no" ?>
            <Mission xmlns="http://ProjectMalmo.microsoft.com" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
                ...
                <ServerSection>
                    ...
                    <ServerHandlers>''' \
                        + get_world_path() + populateObstacles() + '''
    ...
    def log_returns(self):
        ...
        plt.ylabel('Return')
        plt.xlabel('Steps')
        plt.savefig('returns_ppo_lava_tf_2.png')

        with open('returns_ppo_lava_tf_2.txt', 'w') as f:
...
if __name__ == '__main__':
    ...
    while True:
        rmtree("/home/vikram/Desktop/uci_stuff/fourth_year/CS_175/MalmoPlatform/Malmo/samples/Python_examples/Red-Light-Green-Light/missions/Python_Examples/world_lava")
        os.mkdir("/home/vikram/Desktop/uci_stuff/fourth_year/CS_175/MalmoPlatform/Malmo/samples/Python_examples/Red-Light-Green-Light/missions/Python_Examples/world_lava")
