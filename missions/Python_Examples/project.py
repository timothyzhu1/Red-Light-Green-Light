...
class DiamondCollector(gym.Env):
    ...
    def __init__(self, env_config):  
        ...
        self.action_space = Box(np.array([-0.5,-1,-1]), np.array([0.5,1,1]))
    ...
    def step(self, action):
        ...
        def momentum(acceleration):
            #increase time val
            for i in range(100):
                self.currVelo += acceleration * 0.01
                if self.currVelo < -1:
                    self.currVelo = -1
        ...
        # command1 = f'move {action[0]}'
        command2 = f'turn {action[1]}'
        command3 = 'attack 1' if action[2] > 0 else 'attack 0'

        
            
        if command3 != 'attack 1':
            momentum(action[0])
            # self.agent_host.sendCommand(command1)
            # self.agent_host.sendCommand(command2)
            # self.agent_host.sendCommand(command3)
    ...
    def get_mission_xml(self):
        def get_random_diamonds():
            out = ""
            for i in range(-self.size, self.size+1):
                for j in range(-self.size, self.size+1):
                    if random.random() < self.reward_density:
                        out += f"<DrawBlock x='{i}'  y='2' z='{j}' type='diamond_ore' />\n"
        ...
        def get_random_lava():
            ...
            for i in range(-self.size, self.size+1):
                for j in range(-self.size, self.size+1):
                    if random.random() < self.penalty_density:
                        out += f"<DrawBlock x='{i}'  y='1' z='{j}' type='lava' />\n"
